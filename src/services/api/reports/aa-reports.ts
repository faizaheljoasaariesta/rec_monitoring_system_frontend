...
export const getProduct = async (): Promise<ProductResponse> => {
  try {
    const response = await axiosInstance.get<ProductResponse>('/aa-iot/product')
    return response.data;
  } catch (error: any) {
    console.error("Error fatching product:", error)
    throw new Error(error.message || "Failed to fetch AA-IOT product");
  }
}
...
export const getAnalytic = async (
  ...
  try {
    ...
    throw new Error(error.message || "Failed to fetch AA-IOT analytic")
  }
}
