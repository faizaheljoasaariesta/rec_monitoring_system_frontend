...
export const getBIQTSummaryDaily = async (
  start: string,
  end: string,
  product?: string,
  ...
  try {
    const response = await axiosInstance.get<SummaryDailyResponse>(
      `/biqt-iot/summary/daily`,
      { params: { start, end, product } }
    )
    return response.data
  } catch (error: any) {
    throw new Error(error.message || 'Failed to fetch BIQT-IoT daily summary')
  }
}

export const getBIQTReportData = async (
  start: string,
  end: string,
  product?: string
...
export const getBIQTSummary = async (
  start: string,
  end: string,
  product?: string,
...
export const getBIQTProduct = async (): Promise<ProductResponse> => {
  try {
    const response = await axiosInstance.get<ProductResponse>('/biqt-iot/product')
    return response.data;
  } catch (error: any) {
    console.error("Error fatching product:", error)
    throw new Error(error.message || "Failed to fetch BIQT-IOT product");
  }
}

export const getBIQTAnalytic = async (
  start: string,
  end: string,
...
export const getBIQTMonthlyAnalytic = async (
  start: string,
  end: string,
